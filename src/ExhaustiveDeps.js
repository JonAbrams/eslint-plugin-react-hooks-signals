    ...
    function visitFunctionWithDependencies(
      ...
      function gatherDependenciesRecursively(currentScope) {
        for (const reference of currentScope.references) {
          ...
          // If this dependency appears to be declared outside the function, ignore it.
          // Unless it's a signal value.
          if (!pureScopes.has(reference.resolved.scope)) {
            const isSignal = dependency.endsWith(".value");
      ...
      if (declaredDependenciesNode.type !== "ArrayExpression") {
        ...
        declaredDependenciesNode.elements.forEach((declaredDependencyNode) => {
          ...
          const isSignalValue = declaredDependency.endsWith(".value");
          if (!isDeclaredInComponent && !isSignalValue) {
  ...
  function scanTreeRecursively(node, missingPaths, satisfyingPaths, keyToPath) {
    node.children.forEach((child, key) => {
      ...
      if (child.isSatisfiedRecursively) {
        const isSignal = child.children.has("value");
        if (child.isSubtreeUsed && !isSignal) {
